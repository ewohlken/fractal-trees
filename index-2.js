...
function loop(time) {
  ...
  rotationsPerLine = 10 - Math.floor(Math.abs(Math.sin(progress) * 10));
	console.log(rotationsPerLine);
...
function draw(xStart, yStart, length, thickness, angle) {
	...
	for(var i = 0; i < rotationsPerLine; i++) {
		ctx.beginPath();
		ctx.rotate(angle * Math.PI / 180);

		var _thickness = thickness - ((thickness - (thickness*0.8))/rotationsPerLine) * i;
		...
		ctx.moveTo(0, 0);
		ctx.lineTo(0, -length/rotationsPerLine);
		ctx.stroke();

		ctx.translate(0, -lineSpacing-(length/rotationsPerLine));
	}
